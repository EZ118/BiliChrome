class HomeView {
    constructor() {
        ...
        // 初始化事件绑定
        $(document)
            .off("click", ".tabbar .tab")
            .on("click", ".tabbar .tab", (event) => {
        ...
        $.get(
            ...
            (tjlist) => {
                const vidList = tjlist.data.item.map((item) => ({
                    ...
                    pic: item.pic,
                    title: item.title,
                    desc: `[${formatDuration(item.duration)}] ${item.title}`,
                    author: { uid: item.owner.mid, name: item.owner.name },
                }));

                $(".tab_container").append(
                    card.video(vidList) +
                        "<div class='flex_hint_stripe'>向下滚动以加载新内容</div>",
                );
                $("#dynamic_loader").hide();
    ...
    getPopularVideos() {
        ...
        $.get(
            `https://api.bilibili.com/x/web-interface/popular?ps=40&pn=${this.currentPage}`,
            (tjlist) => {
    ...
    getLiveRooms() {
        ...
        $.get(
            `https://api.live.bilibili.com/room/v1/Index/getShowList?page=${this.currentPage}&page_size=20&platform=web`,
            (tjlist) => {
